...
Conflicts: %name-icinga-master %name-icinga-agent
...
Requires: icinga2-common
Provides: nagwad-icinga-templates = %version
Conflicts: %name-icinga %name-icinga-agent
...
Requires: icinga2-common
Provides: nagwad-icinga-templates = %version
Conflicts: %name-icinga-master %name-icinga
...
%check
%make_build check || (cat test/test-suite.log; exit 1)
...
- Fix: Get rid of trailing space in a message addon.
- Fixed a warning from 'ps' if /proc is not mounted.
- Fixed 10-eperm post-filter: Report skip iff corresponding audit
  data found.
- device.regexp: Added expression for messages generated by
  usb-guard.
- Set up a testing suite (run some tests when building the package).
- Support TEST_BOOTID conf param in --test mode.
- Add support for FILTERDIR configuration parameter.
- Fixed duplicate integalert events.
- Added -c | --config option to run nagwad with an alternative
  configuration file.
- Some improvements to run nagwad in test mode (testing boot-ID,
  don't cleanup maxage logs, run with an alternative configuration
  file).
