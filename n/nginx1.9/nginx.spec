...
Source: %url/%oname-%version.tar
Source1: %oname.conf.in
Source2: %oname.init
Source3: %oname.logrotate.in
Source5: %oname.sysconfig
Source6: default.conf
Source7: cache_purge.tar
Source8: ngx_ctpp2.tar
Source9: %oname.service
...
%define nginx_etc %_sysconfdir/%oname
%define nginx_spool %_spooldir/%oname
%define nginx_log %_logdir/%oname

%define configs %buildroot{%_unitdir/%oname.service,%_sysconfdir/logrotate.d/%oname,%nginx_etc/{%oname.conf,sites-available.d/default.conf}}
...
mkdir -p %buildroot%_lockdir/%oname
...
rm -f %buildroot%nginx_etc/%oname.conf
install -pD -m644 %SOURCE1 %buildroot%nginx_etc/%oname.conf
install -pD -m755 %SOURCE2 %buildroot%_initdir/%oname
install -pD -m644 %SOURCE3 %buildroot%_sysconfdir/logrotate.d/%oname
install -pD -m644 %SOURCE5 %buildroot%_sysconfdir/sysconfig/%oname
install -pD -m644 %SOURCE6 %buildroot%nginx_etc/sites-available.d/default.conf
install -pD -m644 %SOURCE9 %buildroot%_unitdir/%oname.service
...
cp -a CHANGES CHANGES.ru %buildroot%_docdir/%oname-%version/
...
%attr(0700,root,root) %dir %_lockdir/%oname
...
%changelog
* Wed Nov 25 2015 Igor Vlasenko <viy@altlinux.ru> 1:1.9.3-alt1.1
...
- Enabled ngx_http_auth_request_module

* Wed May 13 2015 Eugeny A. Rostovtsev (REAL) <real at altlinux.org> 1.9.0-alt0.M70T.2
- Enabled modules: http_degradation, stream, stream_ssl

* Tue May 12 2015 Eugeny A. Rostovtsev (REAL) <real at altlinux.org> 1.9.0-alt0.M70T.1
- Version 1.9.0 for t7/branch

* Fri May 08 2015 Eugeny A. Rostovtsev (REAL) <real at altlinux.org> 1.8.0-alt0.M70T.2
- Enabled PCRE JIT & AIO multithreads

* Fri May 08 2015 Eugeny A. Rostovtsev (REAL) <real at altlinux.org> 1.8.0-alt0.M70T.1
- Built for t7/branch
- Added nginx-sticky-module-ng

* Fri Apr 24 2015 Denis Smirnov <mithraen@altlinux.ru> 1.8.0-alt1
- 1.8.0
