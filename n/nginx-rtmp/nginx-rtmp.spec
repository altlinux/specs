...
Source1: %_name.conf.in
Source2: %_name.init
Source3: %_name.logrotate.in
Source5: %_name.sysconfig
Source6: default.conf
Source7: cache_purge.tar
Source8: ngx_ctpp2.tar
Source9: %_name.service
...
%define nginx_etc %_sysconfdir/%_name
%define nginx_spool %_spooldir/%_name
%define nginx_log %_logdir/%_name

%define configs %buildroot{%_unitdir/%name.service,%_sysconfdir/logrotate.d/%_name,%nginx_etc/{%_name.conf,sites-available.d/default.conf}}
...
mkdir -p %buildroot%_lockdir/%_name
...
rm -f %buildroot%nginx_etc/%_name.conf
install -pD -m644 %SOURCE1 %buildroot%nginx_etc/%_name.conf
install -pD -m755 %SOURCE2 %buildroot%_initdir/%name
install -pD -m644 %SOURCE3 %buildroot%_sysconfdir/logrotate.d/%_name
install -pD -m644 %SOURCE5 %buildroot%_sysconfdir/sysconfig/%_name
...
%attr(0700,root,root) %dir %_lockdir/%_name
...
%changelog
* Wed Nov 25 2015 Igor Vlasenko <viy@altlinux.ru> 1.4.5-alt2.1.1
