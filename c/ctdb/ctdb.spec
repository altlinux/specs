...
Patch1: 0001-Add-missing-eval-to-m4-script.patch
Patch2: 0001-Extract-some-init-functions-into-a-separate-file.patch
Patch3: 0001-Fixes-for-various-issues-found-by-Coverity.patch
Patch4: 0001-IPv6-neighbor-solicit-cleanup.patch
Patch5: 0001-Set-FD_CLOEXEC-for-epoll-file-descriptors.patch
Patch6: 0002-Add-systemd-support.patch
Patch7: ctdb-no_default_runlevel.patch
...
%description devel
Include files to develop CTDB applications. CTDB is a cluster implementation
of the TDB database used by Samba and other projects to store temporary data.
...
Static library to develop CTDB applications. CTDB is a cluster implementation
of the TDB database used by Samba and other projects to store temporary data.
If an application is already using TDB for temporary data it is very easy to
convert that application to be cluster aware and use CTDB instead.
...
CFLAGS="-DCTDB_VERS=\"%version-%release\"" %configure
...
install -m755 config/ctdb.init %buildroot%_initdir/ctdb
...
%attr(755,root,root) %_initdir/ctdb
...
%changelog
* Thu Jan 26 2012 Vitaly Kuznetsov <vitty@altlinux.ru> 1.2.39-alt1
- 1.2.39 with Fedora patches
- remove dependency from ctdb-devel to ctdb

* Thu Aug 11 2011 Vitaly Kuznetsov <vitty@altlinux.ru> 1.0.114.3-alt1
- 1.0.114.3

* Tue Jul 06 2010 Vitaly Kuznetsov <vitty@altlinux.ru> 1.0.113-alt3
- add condstop target
- remove Vendor tag

* Wed May 12 2010 Vitaly Kuznetsov <vitty@altlinux.ru> 1.0.113-alt2
- fix license

* Thu Jan 21 2010 Anton V. Boyarshinov <boyarsh@altlinux.ru> 1.0.113-alt1
- build for sisyphus
...
* Mon Jul 7 2008 : Version 1.0.44
 - Add a CTDB_VALGRIND option to %_sysconfdir/sysconfig/ctdb to make it start
...
* Thu May 22 2008 : Version 1.0.39
 - Moved a CTDB_MANAGES_NFS, CTDB_MANAGES_ISCSI and CTDB_MANAGES_CSFTPD
   into %_sysconfdir/sysconfig/ctdb
...
* Thu Apr 10 2008 : Version 1.0.33
 - Add facilities to include site local adaptations to the eventscript
   by %_sysconfdir/ctdb/rc.local which will be read by all eventscripts.
