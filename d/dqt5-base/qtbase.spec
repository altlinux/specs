...
%add_findreq_skiplist %_dqt5_plugindir/platformthemes/*.so
# find libraries
...
%package -n dqt5-qtbase
Summary: dqt5-qtbase compatibility package
Group: System/Libraries
BuildArch: noarch
...
%description -n dqt5-qtbase
dqt5-qtbase compatibility package

%package -n dqt5-qtbase-gui
Summary: dqt5-qtbase-gui compatibility package
Group: System/Libraries
BuildArch: noarch
...
Provides: dqt5-qtbase-x11 = %version-%release
%description -n dqt5-qtbase-gui
dqt5-qtbase-gui compatibility package
...
./configure -v \
    ...
    -confirm-license \
    -prefix %_dqt5_prefix \
    -extprefix %_dqt5_prefix \
    -archdatadir %_dqt5_archdatadir \
    -bindir %_dqt5_bindir \
    -datadir %_dqt5_datadir \
    -docdir %_dqt5_docdir \
    -examplesdir %_dqt5_examplesdir \
    -headerdir %_dqt5_headerdir \
    -importdir %_dqt5_importdir \
    -qmldir %_dqt5_qmldir \
    -libdir %_dqt5_libdir \
    -R %_dqt5_libdir \
    -libexecdir %_dqt5_libexecdir \
    -plugindir %_dqt5_plugindir \
    -sysconfdir %_dqt5_sysconfdir \
    -translationdir %_dqt5_translationdir \
    -platform %platform \
    -release \
    -shared \
    -pkg-config \
...
mkdir -p %buildroot/%_dqt5_headerdir/QtXcb
install -m 0644 src/plugins/platforms/xcb/*.h %buildroot/%_dqt5_headerdir/QtXcb/

# uninstall optflags
sed -i "s|^\s*QMAKE_CFLAGS_OPTIMIZE\s*=.*$|QMAKE_CFLAGS_OPTIMIZE = -O2|" %buildroot%_dqt5_archdatadir/mkspecs/common/gcc-base.conf
sed -i "s|^\s*QMAKE_CFLAGS_OPTIMIZE_FULL\s*=.*$|QMAKE_CFLAGS_OPTIMIZE_FULL = -O3|" %buildroot%_dqt5_archdatadir/mkspecs/common/gcc-base.conf

# remove macos staff
rm -rf %buildroot/%_dqt5_archdatadir/mkspecs/features/uikit ||:

# create/own dirs
mkdir -p %buildroot/{%_dqt5_archdatadir/mkspecs/modules,%_dqt5_importdir,%_dqt5_qmldir,%_dqt5_libexecdir,%_dqt5_translationdir,%_dqt5_docdir,%_dqt5_examplesdir}
mkdir -p %buildroot/%_dqt5_plugindir/{accessible,iconengines,script,styles}/

# debug logging config
mkdir -p %buildroot/%_sysconfdir/dqt5/
cat >%buildroot/%_sysconfdir/dqt5/qtlogging.ini <<__EOF__
[Rules]
*.debug=false
qt.qpa.xcb.xcberror.warning=false
__EOF__
ln -s `relative %_sysconfdir/dqt5/qtlogging.ini %_dqt5_datadir/qtlogging.ini` %buildroot/%_dqt5_datadir/qtlogging.ini

# remove .la files
rm -rf %buildroot/%_dqt5_libdir/*.la

# .pc
mkdir -p %buildroot/%_dqt5_libdir/pkgconfig/
cat >%buildroot/%_dqt5_libdir/pkgconfig/Qt%major.pc<<__EOF__
prefix=%_dqt5_prefix
archdatadir=%_dqt5_archdatadir
bindir=%_dqt5_bindir
datadir=%_dqt5_datadir
...
libdir=%_dqt5_libdir
libexecdir=%_dqt5_libexecdir
moc=%_dqt5_bindir/moc
plugindir=%_dqt5_plugindir
qmake=%_dqt5_bindir/qmake
settingsdir=%_dqt5_settingsdir
sysconfdir=%_dqt5_sysconfdir
translationdir=%_dqt5_translationdir
...
%%_dqt5 %_dqt5
%%_dqt5_epoch %{?epoch}%{!?epoch:0}
%%_dqt5_version %version
%%_dqt5_evr %EVR
__EOF__
...
ln -s `relative %_dqt5_archdatadir/$d %_dqt5_prefix/$d` %buildroot/%_dqt5_prefix/$d
done
ln -s `relative %_dqt5_docdir %_dqt5_prefix/doc` %buildroot/%_dqt5_prefix/doc

# install binaries to %_bindir
mkdir %buildroot/%_bindir
ls -1d %buildroot/%_dqt5_bindir/* | \
while read f ; do
    [ -f "$f" ] || continue
    fname=`basename $f`
    mv $f %buildroot/%_bindir/$fname-%gname
    ln -s `relative %_bindir/$fname-%gname %_dqt5_bindir/$fname-%gname` %buildroot/%_dqt5_bindir/$fname
done

# qtchooser conf
%if_enabled qtchooser
  mkdir -p %buildroot%_sysconfdir/xdg/dqt5/qtchooser
  pushd    %buildroot%_sysconfdir/xdg/dqt5/qtchooser
    echo "%_dqt5_bindir" >  %{gname}.conf
    echo "%_dqt5_prefix" >> %{gname}.conf
  popd
%endif
...
mkdir -p %buildroot/%_dqt5_prefix/lib
ls -1d %buildroot/%_dqt5_libdir/*.{a,so,prl} 2>/dev/null | \
while read f ; do
    [ -d "$f" ] && continue
    fname=`basename $f`
    ln -s `relative $f %buildroot/%_dqt5_prefix/lib/$fname` %buildroot/%_dqt5_prefix/lib/$fname
done
# link includes into qt prefix
ln -s `relative %buildroot/%_dqt5_headerdir %buildroot/%_dqt5_prefix/include` %buildroot/%_dqt5_prefix/include

%if 0%{?qtchooser}
%files
# not editable config files, so not using %%config here
%dir %_sysconfdir/xdg/dqt5/qtchooser
%_sysconfdir/xdg/dqt5/qtchooser/*.conf
%endif
...
%files -n dqt5-qtbase
%files -n dqt5-qtbase-gui

%files common
%doc LICENSE.*
%dir %_sysconfdir/dqt5/
%dir %_dqt5_docdir/
%dir %_dqt5_archdatadir/
%dir %_dqt5_examplesdir/
%dir %_dqt5_importdir/
%dir %_dqt5_qmldir/
%dir %_dqt5_translationdir/
%dir %_dqt5_prefix/
%dir %_dqt5_prefix/doc/
%dir %_dqt5_prefix/imports/
%dir %_dqt5_prefix/lib/
%dir %_dqt5_prefix/libexec/
%dir %_dqt5_prefix/plugins/
%if "%_dqt5_prefix" != "%_dqt5_datadir"
%dir %_dqt5_datadir/
%endif
%dir %_dqt5_libexecdir/
%dir %_dqt5_plugindir/
%dir %_dqt5_plugindir/accessible/
%dir %_dqt5_plugindir/bearer/
%dir %_dqt5_plugindir/egldeviceintegrations/
%dir %_dqt5_plugindir/generic/
%dir %_dqt5_plugindir/iconengines/
%dir %_dqt5_plugindir/imageformats/
%dir %_dqt5_plugindir/platforminputcontexts/
%dir %_dqt5_plugindir/platforms/
%dir %_dqt5_plugindir/platformthemes/
%dir %_dqt5_plugindir/printsupport/
%dir %_dqt5_plugindir/script/
%dir %_dqt5_plugindir/sqldrivers/
%dir %_dqt5_plugindir/styles/
%dir %_dqt5_plugindir/xcbglintegrations/
%config(noreplace) %_sysconfdir/dqt5/qtlogging.ini
%_dqt5_datadir/qtlogging.ini

%files doc
%if %qdoc_found
%doc %_dqt5_docdir/*
%exclude %_dqt5_docdir/config/
%exclude %_dqt5_docdir/global/
%endif
%_dqt5_examplesdir/*
...
%_dqt5_bindir/qdbuscpp2xml*
%_bindir/qdbusxml2cpp*
%_dqt5_bindir/qdbusxml2cpp*
%_bindir/fixqt4headers.pl*
%_dqt5_bindir/fixqt4headers.pl*
%_bindir/qmake*
%_dqt5_bindir/qmake*
%_bindir/rcc*
%_dqt5_bindir/rcc*
...
%_dqt5_bindir/qvkgen*
%_bindir/tracegen*
%_dqt5_bindir/tracegen*
%dir %_dqt5_headerdir
%dir %_dqt5_prefix/include/
%_dqt5_headerdir/Qt*/
%dir %_dqt5_prefix/mkspecs/
%_dqt5_archdatadir/mkspecs/
%_dqt5_prefix/lib/libQt%{major}*.prl
%_dqt5_libdir/libQt%{major}*.prl
%_dqt5_prefix/lib/libQt%{major}*.so
%_dqt5_libdir/libQt%{major}*.so
%dir %_dqt5_libdir/cmake/
%_dqt5_libdir/cmake/Qt%{major}*/
%dir %_dqt5_libdir/metatypes/
%_dqt5_libdir/metatypes/qt5*.json
%_dqt5_libdir/pkgconfig/Qt%{major}.pc
%_dqt5_libdir/pkgconfig/Qt%{major}Concurrent.pc
%_dqt5_libdir/pkgconfig/Qt%{major}Core.pc
%_dqt5_libdir/pkgconfig/Qt%{major}DBus.pc
%_dqt5_libdir/pkgconfig/Qt%{major}Gui.pc
%_dqt5_libdir/pkgconfig/Qt%{major}Network.pc
%_dqt5_libdir/pkgconfig/Qt%{major}OpenGL.pc
%_dqt5_libdir/pkgconfig/Qt%{major}PrintSupport.pc
%_dqt5_libdir/pkgconfig/Qt%{major}Sql.pc
%_dqt5_libdir/pkgconfig/Qt%{major}Test.pc
%_dqt5_libdir/pkgconfig/Qt%{major}Widgets.pc
%_dqt5_libdir/pkgconfig/Qt%{major}Xml.pc

%files devel-static
%_dqt5_libdir/libQt%{major}*.a
%_dqt5_prefix/lib/libQt%{major}*.a
%_dqt5_libdir/pkgconfig/Qt%{major}OpenGLExtensions.pc
...
#%_dqt5_plugindir/sqldrivers/libqsqlite.so
#%endif

%if_enabled sql_ibase
%files -n %gname-sql-interbase
%_dqt5_plugindir/sqldrivers/libqsqlibase.so
%endif

%files -n %gname-sql-mysql
%_dqt5_plugindir/sqldrivers/libqsqlmysql.so

%if_enabled sql_odbc
%files -n %gname-sql-odbc
%_dqt5_plugindir/sqldrivers/libqsqlodbc.so
%endif

%if_enabled sql_pgsql
%files -n %gname-sql-postgresql
%_dqt5_plugindir/sqldrivers/libqsqlpsql.so
%endif

%if_enabled sql_tds
%files -n %gname-sql-tds
%_dqt5_plugindir/sqldrivers/libqsqltds.so
%endif

%if_enabled sql_sqlite2
%files -n %gname-sql-sqlite2
%_dqt5_plugindir/sqldrivers/libqsqlite2.so
%endif

%files -n lib%{gname}-core
%_dqt5_libdir/libQt%{major}Core.so.*

%files -n lib%{gname}-concurrent
%_dqt5_libdir/libQt%{major}Concurrent.so.*

%files -n lib%{gname}-dbus
%_dqt5_libdir/libQt%{major}DBus.so.*

%files -n lib%{gname}-gui
%_dqt5_libdir/libQt%{major}Gui.so.*
%_dqt5_plugindir/egldeviceintegrations/*
%_dqt5_plugindir/generic/*
%_dqt5_plugindir/imageformats/*
%_dqt5_plugindir/platforminputcontexts/*
%_dqt5_plugindir/platforms/*
%_dqt5_plugindir/platformthemes/*
%_dqt5_plugindir/xcbglintegrations/*

%files -n lib%{gname}-network
%_dqt5_libdir/libQt%{major}Network.so.*
%_dqt5_plugindir/bearer/*

%files -n lib%{gname}-opengl
%_dqt5_libdir/libQt%{major}OpenGL.so.*

%files -n lib%{gname}-printsupport
%_dqt5_libdir/libQt%{major}PrintSupport.so.*
%_dqt5_plugindir/printsupport/*

%files -n lib%{gname}-sql
%_dqt5_libdir/libQt%{major}Sql.so.*
%_dqt5_plugindir/sqldrivers/libqsqlite.so

%files -n lib%{gname}-test
%_dqt5_libdir/libQt%{major}Test.so.*

%files -n lib%{gname}-widgets
%_dqt5_libdir/libQt%{major}Widgets.so.*
#%_dqt5_plugindir/accessible/*

%files -n lib%{gname}-xml
%_dqt5_libdir/libQt%{major}Xml.so.*

%files -n lib%{gname}-eglfsdeviceintegration
%_dqt5_libdir/libQt%{major}EglFSDeviceIntegration.so.*

%files -n lib%{gname}-xcbqpa
%_dqt5_libdir/libQt%{major}XcbQpa.so.*

%files -n lib%{gname}-eglfskmssupport
%_dqt5_libdir/libQt%{major}EglFsKmsSupport.so.*
