# check currently doesn't work for aarch64
%ifarch aarch64
%def_without check
%endif
...
gcc %optflags -s -static -xc -o init - <<__EOF__
#include <unistd.h>
...
__EOF__
echo init | cpio -H newc -o > initrd.img
qemu_arch=%_arch
qemu_opts=""
console=ttyS0
%ifarch aarch64
...
%ifarch aarch64
# kernel/drivers/crypto/tegra/tegra-se.ko needs kernel/drivers/gpu/host1x/host1x.ko
# so move it to kernel-modules-drm package
%exclude %modules_dir/kernel/drivers/crypto/
...
- config-aarch64: Disable panthor driver
- config: Disable ZSWAP
- config: Disable CONFIG_BLK_CGROUP_IOLATENCY
- config: Disable InfiniBand user-space management and RDMA

* Mon Jun 30 2025 Alexander Stepchenko <geochip@altlinux.org> 6.12.35-alt1
- Update to v6.12.35 (2025-06-27)
...
- config: Enable CONFIG_NFT_CONNLIMIT=y
- config: Disable IMA support
...
- Remove checkinstall package.

* Thu Jan 16 2025 Alexander Stepchenko <geochip@altlinux.org> 6.12.9-alt1
- Remove accelerators related modules
- Remove RADIO_TEA575X module installation
- Remove NVIDIA and DisplayPort Alternate Mode driver modules
- Remove modules packages nouveau and staging
- Only build for x86_64 and aarch64
- Use config similiar to Talos Linux
