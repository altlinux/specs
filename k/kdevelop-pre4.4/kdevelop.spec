...
%define req_big %kdevelop = %serial:%version-%release %kdevelop-for-debug = %serial:%version-%release %kdevelop-for-kde = %serial:%version-%release %kdevelop-for-qt = %serial:%version-%release
%define req_big_features \
  * debug module \
...
# this one too :/
%define req_maxi %req_big %kdevplatform-subversion >= %build_req_kdeplatform_min %kdevplatform-git >= %build_req_kdeplatform_min %kdevplatform-cvs >= %build_req_kdeplatform_min %kdevelop-for-php >= %req_kdev_php_min
...
Requires: %kdevelop-base = %serial:%version-%release
...
Patch11: kubuntu_01_appmenu_fix.diff
...
Requires: %kdevelop-common = %serial:%version-%release
...
# Only stable package replaces unstable counterpart
%if_stable Obsoletes: %{kdevelop_other}-base < %serial:%version-%release

# Drop previous -unstable
Conflicts: kdevelop-unstable-base

#Requires: perl bash libSDL-devel
#Requires: autoconf automake make cmake
Requires: make cmake
# enscript
#Requires: libssl-devel libjpeg-devel xorg-devel
#Requires: sgml-tools gettext zlib-devel ctags gettext-tools
#Requires: libtool patch doxygen
%description base
...
%package maxi
Group: Development/Tools
Summary: Meta-package for "Maxi" KDevelop installation
Requires: %req_maxi
...
%package big
Group: Development/Tools
Summary: Meta-package for an extended KDevelop installation
Requires: %req_big
...
%files maxi
%files big
%files for-debug -f kdevgdb.lang
%_K4lib/kdevgdb.so
%_K4apps/kdevgdb
...
%files for-qt -f kdevqthelp.lang
...
%if_enabled okteta
%files okteta -f kdevokteta.lang
