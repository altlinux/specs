...
install -d -m 0755 %buildroot%klibcdir/include/scsi
install -p -m 0644 %_includedir/scsi/* %buildroot%klibcdir/include/scsi/
sed -i '/^#include/s|features.h|sys/stat.h|' %buildroot%klibcdir/include/scsi/{scsi,sg}.h

install -p -m 0755 %name-find-requires %buildroot%klibcdir/

# Install the docs
install -d -m 0755 %buildroot{%bindocdir,%libdocdir}
install -p -m 0644 README usr/%name/{README.%name,arch/README.%name.arch} %buildroot%libdocdir/

install -p -m 0644 usr/kinit/ipconfig/README.ipconfig %buildroot%bindocdir/
for f in usr/gzip/COPYING usr/gzip/README usr/kinit/README; do
...
klibc_soname=$(cd %buildroot/%_lib && ls -1 %name-*.so)
install -d -m 0755 %buildroot/lib/mkinitrd/%name/{bin,sbin,%_lib}
ln -s /%_lib/"$klibc_soname" %buildroot/lib/mkinitrd/%name/%_lib/"$klibc_soname"
...
for p in poweroff reboot; do
	ln %buildroot/lib/mkinitrd/%name/sbin/{halt,$p}
done
