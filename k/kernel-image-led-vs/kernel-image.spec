...
%{?_enable_lto:%set_disable modversions}
...
%endif
	...
	%{?_disable_compat:sysctl_syscall acpi_procfs_power compat_vdso i2c_compat proc_pid_cpuset sysfs_deprecated edac_legacy_sysfs x86_acpi_cpufreq_cpb} \
...
%make_kernel bzImage modules
%{?_enable_modversions:%{?extra_mods:%make_kernel -f Makefile.external %extra_mods && echo "External modules built"}}
%endif
...
- updated:
  ...
  + feat-fs-aufs
- enabled zfs extmod for xen kernel

* Sun Jun 15 2014 Led <led@altlinux.ru> 3.15.0-alt1
- initial build of 3.15 based on 3.14.7-alt2
