...
%setup
%patch -p1
sed -i	-e '/^udev_udevd_SOURCES/,/^$/d' \
	-e '/^[[:blank:]]*udev\/udevd /d' \
	-e '/^[[:blank:]]*udev\/udevadm\.c /s|c |&udev/udevd.c |' \
	Makefile.am
for s in d adm; do
	sed -i "/^int main(/s/main(/udev$s(/" udev/udev$s.c
done
cat >> udev/udevadm.c <<__EOF__

extern int udevd(int argc, char *argv[]);

int main(int argc, char *argv[])
{
	return (argv[0][strlen(argv[0]) - 1] == 'd' ? udevd : udevadm)(argc, argv);
}
__EOF__
...
%install
%makeinstall_std udevrulesdir=%_sysconfdir/udev/initramfs-rules.d
ln -s udevadm %buildroot/lib/mkinitrd/udev/sbin/udevd
