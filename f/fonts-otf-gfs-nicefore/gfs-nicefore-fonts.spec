...
%define fontpkgname gfs-nicefore-fonts
...
Source10: 61-gfs-nicefore-fonts.xml
...
# fontbuild 
fontnames=$(
  for font in 'GFSNicefore.otf'; do
...
%install
echo "Installing "gfs-nicefore-fonts
echo "" > "gfs-nicefore-fonts.list"
install -m 0755 -vd %buildroot%_fontsdir/otf/gfs-nicefore/
echo "%%dir %_fontsdir/otf/gfs-nicefore" >> "gfs-nicefore-fonts.list"
install -m 0644 -vp "GFSNicefore.otf" %buildroot%_fontsdir/otf/gfs-nicefore/
echo \"%_fontsdir/otf/gfs-nicefore//$(basename "GFSNicefore.otf")\" >> 'gfs-nicefore-fonts.list'
(

  IFS= lines=$(
    for fontconfng in '%SOURCE10'; do
      gen-fontconf -x "${fontconfng}" -w -f 'GFSNicefore.otf'
    done
  )
  ...
  for fontconf in  "${newfontconfs[@]}"; do
    if [[ -n $fontconf ]] ; then
      ...
      echo \"%{_fontconfig_templatedir}/$(basename "${fontconf}")\"                  >> "gfs-nicefore-fonts.list"
      ln -vsr "%{buildroot}%{_fontconfig_templatedir}/$(basename "${fontconf}")" "%{buildroot}%{_fontconfig_confdir}"
      echo "%%config(noreplace)" \"%{_fontconfig_confdir}/$(basename "${fontconf}")\" >> "gfs-nicefore-fonts.list"
    fi
  done
)

install -m 0755 -vd "%{buildroot}%{_metainfodir}"
for fontappstream in 'org.altlinux.gfs-nicefore-fonts.metainfo.xml'; do
  install -m 0644 -vp "${fontappstream}" "%{buildroot}%{_metainfodir}"
  echo \"%{_metainfodir}/$(basename "${fontappstream}")\" >> "gfs-nicefore-fonts.list"
done

for fontdoc in 'OFL-FAQ.txt'; do
  echo %%doc "'${fontdoc}'" >> "gfs-nicefore-fonts.list"
done

for fontlicense in 'OFL.txt'; do
  echo %%doc "'${fontlicense}'" >> "gfs-nicefore-fonts.list"
done

%check
# fontcheck 
grep -E '^"%{_fontconfig_templatedir}/.+\.conf"' 'gfs-nicefore-fonts.list' \
  | xargs -I{} -- sh -c "xmllint --loaddtd --valid     --nonet '%{buildroot}{}' >/dev/null && echo %{buildroot}{}: OK"
grep -E '^"%{_datadir}/metainfo/.+\.xml"'        'gfs-nicefore-fonts.list' \
  | xargs -I{} --        appstream-util validate-relax --nonet '%{buildroot}{}'

%files -n fonts-otf-gfs-nicefore -f gfs-nicefore-fonts.list
...
* Thu Mar 22 2012 Igor Vlasenko <viy@altlinux.ru> 20080303-alt3_8
