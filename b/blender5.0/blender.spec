...
# FIXME: remove aarch64 for now as build platform is very unstable
...
Patch20: blender-2.77-alt-enable-localization.patch
Patch21: blender-2.80-alt-use-system-glog.patch
Patch22: blender-2.90-alt-non-x86_64-linking.patch
...
Patch39: blender-4.5.0-alt-numpy-inc.patch
Patch40: blender-alt-optix-inc.patch
...
BuildRequires: pipewire-libs-devel librubberband-devel
...
%add_python3_req_skip _blendfile_header
...
Obsoletes: %project-i18n, %{project}4.4 %{project}4.5
...
Obsoletes: %{project}4.4-cycles-hip-kernels %{project}4.5-cycles-hip-kernels
...
Obsoletes: %{project}4.4-cycles-hiprt-kernels %{project}4.5-cycles-hiprt-kernels
Conflicts: %project-cycles-hip-kernels
...
Obsoletes: %{project}4.4-cycles-nvidia-kernels, %{project}4.5-cycles-nvidia-kernels %{project}4.5-cycles-cuda-kernels
...
Group: System/Libraries
Requires: %name = %EVR, %{project}5.0-cycles-cuda-kernels = %EVR, libnvoptix
Obsoletes: %{project}4.5-cycles-optix-kernels
...
%patch20 -p1 -b .enable-l10n
%patch21 -p1 -b .system-glog
%patch22 -p1 -b .non-x86_64-linking
#%%patch30 -p1
%patch31 -p1 -b .alt-osl-shader-dir
#%%patch32 -p1
%ifarch aarch64
...
%patch36 -p1 -b .system-draco
%if_with hiprt
%patch37 -p1 -b .hiprt-2.5
%patch38 -p1 -b .hiprt-optflags
%endif
%patch39 -p1 -b .alt-numpy-inc
...
rm -rf extern/{Eigen3,glew,gflags,glog,draco}
...
- cleanup build flags.
- aarch64: disable for now (due unstable build platform).

* Tue Nov 25 2025 L.A. Kostis <lakostis@altlinux.ru> 4.5.5-alt2
- aarch64: enable cuda/optix.
- cycles-optix-kernels: added dependency to cuda-kernels.
...
- build with manifold and MaterialX.
- build: use lfs tarball hack to unpack needed assets.
...
- Update hiprt requires.
- hiprt: hardcode clang-rocm compiler.
- cuda: build with gcc13.

* Mon Oct 14 2024 L.A. Kostis <lakostis@altlinux.ru> 4.4.0-alt0.1.g191b347f581
- 4.4.0 GIT 191b347f581.
- Enable hiprt back.
- Disable cuda for now (needs gcc downgrade).
- Build separately from main blender package.

* Wed Aug 28 2024 L.A. Kostis <lakostis@altlinux.ru> 4.2.1-alt0.1
- Update to 4.2.1.
- remove -manpage patch (upstream has a fix).

* Mon Jul 22 2024 L.A. Kostis <lakostis@altlinux.ru> 4.2.0-alt0.1
- Update to 4.2.0.
...
- added -DGLOG_USE_GLOG_EXPORT to fix build with glog 0.7.0 (tnx rider@)
