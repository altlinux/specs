...
%mvn_compat_version : %{version} 3 2.1.3
...
install -m 755 bin/bmsubmit.sh $RPM_BUILD_ROOT%{bindir}/bmsubmit3
install -m 755 bin/bminstall.sh  $RPM_BUILD_ROOT%{bindir}/bminstall3
install -m 755 bin/bmjava.sh  $RPM_BUILD_ROOT%{bindir}/bmjava3
install -m 755 bin/bmcheck.sh  $RPM_BUILD_ROOT%{bindir}/bmcheck3

for f in bmsubmit3 bmjava3 bminstall3 bmcheck3; do
cat > $RPM_BUILD_ROOT%{_bindir}/${f} << EOF
#!/bin/sh

export BYTEMAN_HOME=%apphomedir
...
%changelog
* Sat Jul 13 2019 Igor Vlasenko <viy@altlinux.ru> 3.0.6-alt2_5jpp8
- compat package
