...
%define apr_ver 1.4.6-alt1
%define apu_name libaprutil1
%define apu_ver 1.5.1-alt1
...
Source: %rname-%version%svn_ver_pre.tar.bz2
Source1: %rname.el
Source2: %rname-config-1.4.0.tar
...
Patch1: %rname-1.7.8-alt-autogen-fix.patch
Patch4: %rname-1.5.4-alt-perl-DESTROY.patch
Patch5: %rname-1.7.8-alt-custom-libtool.patch
Patch6: %rname-1.7.8-alt-kwallet-build.patch

# http://bugs.gentoo.org/show_bug.cgi?id=219959
Patch16: %rname-1.6.0-gentoo-java-headers.patch

# http://svn.haxx.se/dev/archive-2008-07/0494.shtml
Patch17: %rname-1.6.6-deb-ssh-no-controlmaster.patch
...
Conflicts: lib%rname-devel-static
...
Group: Development/Other
Requires: lib%name = %version-%release
Conflicts: lib%rname-auth-kwallet
...
Group: Development/Other
Requires: lib%name = %version-%release
Conflicts: lib%rname-auth-gnome-keyring
...
Requires: /bin/su
BuildArch: noarch
Conflicts: %rname-server-standalone
...
%prep
%setup -q -n %rname-%version%svn_ver_pre
...
tar -xf %SOURCE2 -C %buildroot%_sysconfdir/%rname
...
%config(noreplace) %_sysconfdir/%rname/config
%config(noreplace) %_sysconfdir/%rname/servers
%_sysconfdir/%rname/README.txt
%_bindir/svn
%_bindir/svnversion
%_bindir/svnrdump
%_bindir/svnsync
...
%changelog
* Wed Feb 20 2013 Andrey Cherepanov <cas@altlinux.org> 1.7.8-alt1
- Build Subversion 1.7.8 as new package (ALT #28469)
