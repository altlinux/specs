%define _unpackaged_files_terminate_build 1
%define import_path github.com/anchore/syft

%def_with check

Name: syft
Version: 1.51.0
Release: alt1

Summary: A CLI tool and Go library for generating an SBOM from container images and filesystems
License: Apache-2.0
Group: Development/Tools
Url: https://github.com/anchore/syft
Vcs: https://github.com/anchore/syft
ExclusiveArch: %go_arches

Source0: %name-%version.tar
Source1: vendor.tar
Source2: .excluded_packages
Source3: list

Patch: %name-%version-alt.patch

BuildRequires(pre): rpm-macros-golang
BuildRequires: rpm-build-golang
BuildRequires: /proc
%if_with check
BuildRequires: git-core
BuildRequires: zip
%endif

%description
A CLI tool and Go library for generating a Software Bill of Materials
(SBOM) from container images and filesystems. Exceptional for
vulnerability detection when used with a scanner like Grype.

%prep
%setup -a1

%build
export BUILDDIR="$PWD/.build"
export IMPORT_PATH="%import_path"
export GOPATH="$BUILDDIR:%go_path"
export CGO_ENABLED=0 
# consumed by golang-build as `go install -ldflags`
export LDFLAGS="-s -w -X main.version=%version  \
                      -X main.gitDescription=v%version \
		      -X main.gitCommit=%(cut -b -8 %SOURCE3) \
		      -X main.buildDate=%(date "+%%Y%%m%%d")"

%golang_prepare

cd .build/src/%import_path
%golang_build ./cmd/syft

%install
export BUILDDIR="$PWD/.build"
export IGNORE_SOURCES=1
%golang_install
ln -sf %_licensedir/Apache-2.0 LICENSE

%check
export GOPATH="$PWD/.build:%go_path"
export GOFLAGS="-mod=vendor"

# several test suites locate the source tree by shelling out to
# `git rev-parse --show-toplevel`; an empty repository is enough for that
git init -q .

cd .build/src/%import_path

# These packages build their fixtures inside container images or drive a
# container runtime directly (docker/podman), neither of which exists in
# the build environment.
go test $(go list ./... | grep -vxFf %SOURCE2)

%files
%doc README.md
%doc --no-dereference LICENSE
%_bindir/%name

%changelog
* Tue Aug 18 2026 Andrey Kuzma <kuzmaav@altlinux.org> 1.51.0-alt1
- Updated to 1.51.0.

* Wed Jul 22 2026 Andrey Kuzma <kuzmaav@altlinux.org> 1.49.0-alt1
- Initial build for ALT Sisyphus.
