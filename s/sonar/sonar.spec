Name: sonar
Version: 3.2
Summary: An open platform to manage code quality
License: LGPLv3+
Url: http://www.sonarqube.org
Packager: Igor Vlasenko <viy@altlinux.ru>
Provides: mvn(org.codehaus.sonar:sonar-core) = 3.2
Provides: mvn(org.codehaus.sonar:sonar-core:pom:) = 3.2
Provides: mvn(org.codehaus.sonar:sonar:pom:) = 3.2
Provides: sonar = 3.2-5.fc23
Requires: java-headless
Requires: jpackage-utils
Requires: mvn(ch.qos.logback:logback-classic)
Requires: mvn(ch.qos.logback:logback-core)
Requires: mvn(com.google.code.findbugs:jsr305)
Requires: mvn(commons-dbcp:commons-dbcp)
Requires: mvn(org.apache.geronimo.specs:geronimo-jta_1.1_spec)
Requires: mvn(org.codehaus.plexus:plexus-classworlds)
Requires: mvn(org.codehaus.sonar:sonar-plugin-api)
Requires: mvn(org.codehaus.sonar:sonar-update-center-common)
Requires: mvn(org.hibernate.common:hibernate-commons-annotations)
Requires: mvn(org.hibernate:hibernate-core:3)
Requires: mvn(org.hibernate:hibernate-ehcache:3)
Requires: mvn(org.hibernate:hibernate-entitymanager:3)
Requires: mvn(org.mybatis:mybatis)
Requires: mvn(org.slf4j:jcl-over-slf4j)
Requires: mvn(org.slf4j:log4j-over-slf4j)
Requires: mvn(org.slf4j:slf4j-api)

BuildArch: noarch
Group: Development/Java
Release: alt0.1jpp
Source: sonar-3.2-5.fc23.cpio

%description
Open source platform for continuous inspection of code quality.

# sometimes commpress gets crazy (see maven-scm-javadoc for details)
%set_compress_method none
%prep
cpio -idmu --quiet --no-absolute-filenames < %{SOURCE0}

%build
cpio --list < %{SOURCE0} | sed -e 's,^\.,,' > %name-list

%install
mkdir -p $RPM_BUILD_ROOT
for i in usr var etc; do
[ -d $i ] && mv $i $RPM_BUILD_ROOT/
done


%files -f %name-list

%changelog
* Sat Feb 06 2016 Igor Vlasenko <viy@altlinux.ru> 3.2-alt0.1jpp
- bootstrap pack of jars created with jppbootstrap script
- temporary package to satisfy circular dependencies

