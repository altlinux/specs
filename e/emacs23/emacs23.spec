...
%else
Source0: %shortname-%emacs_version.tar.bz2
%endif
...
################### Patches  ##########################
# All patches are merged into git tree - see commit info

# ALT's libtinfo; POSIX locks via fcntl convention:
# Patch0: %shortname-22.0.95-alt-system.patch
# commit 5a4c8f44330a8d8c0f6ab62ad49c1e15a00cec97
# Full URL is
# http://git.altlinux.ru/people/eugvv/packages/?p=emacs22.git;a=commit;h=5a4c8f44330a8d8c0f6ab62ad49c1e15a00cec97
...
# commit b5a1fda272543761455f58619e65d7654602e97c

# Patch3: %shortname-22.0.50-alt0.7-clipboard.patch
# commit 393bd84e2ae6a9158a9a40a2ec612d16e80584a3
...
# commit 8dcfc1d5fab4316b9e2f3bc961a07d61b00d7f66

# Patch5: %shortname-22.0.50-Meta-Alt-Ctrl-Hyper-super-discard-state.patch
# commit 089937c72f33588f576cd2472db1f31384741afe

# Patch7: %shortname-22.0.50-pc-select-private-mark.patch
# commit 08be3169b75557c57eea15caad515c29fbb840ef

# Patch14: %shortname-22.0.50-rh-no_rpath.patch
# commit 674e59f29fbb036c6f81afadede2c00501210925

# These two patches make the real work for ukrainisation:
# Patch20: %shortname-22.0.50-alt0.9-uk.patch
# commit b2db32cdd16ab44da617e28c58267203c8742102
# Patch21: leim-22.0.50-uk.patch
# commit b5cc023ede7bafbd108442123867e6d67591372e
# Patch22: leim-22.0.50-belarusian.patch
# commit 15e8964327ce8c696b89b07ec4e6699720f0c9bc
# added belarussian and proprer russianw dictionary support
# Patch23: %shortname-22.0.50-ispell-belarusian.patch
# commit af347339e4fa0d50b237ac5537a3e43c8ed4056b
# Patch24: %shortname-22.0.50-ispell-russianw.patch
# commit a36153e84aa6982c1de76989f7991179aa5dbc77

# Patch33: %shortname-22.0.98-more-cyrillic-support.patch
# commit 0bac38a88cd25c3801167dabeee6ae49a18800f0

# Patch35: %shortname-22.0.50-tutorial-ru.patch
# commit 9ce79c19ccb0628206225dd3f5ce0627dfbd4dc2

# Decode invalid encoded headers
# Patch41: %shortname-22.0.50-gnus-invalid_decode.patch
# commit 2a3a8145b2b973d4eadcedd91f6b2f8489f87111
...
# commit 735beb39653db8f1e01c046b518fdb603f687ce4

# Check save-place-alist elements after loading from ~/.emacs-places
# Patch47: %shortname-22.0.50-saveplace_check_cell.patch
# commit 63b80d6f8125dc879c02b217cad691ab257c1746

# Fix info installation
# Patch50: %shortname-22.0.91-info_install.patch
# commit 14aee500374c1ab62c5289944f5b2362b0701cb1

# Increase PURESIZE
# Patch55: %shortname-22.0.50-incr_puresize.patch
# commit 9135e9e58eef144a3d9fa78435cc2f4294ddae81

# Fix for default path to X client resources (added /etc/X11/app-defaults)
# Patch65: %shortname-22.0.91-x_file_search_path.patch
# commit ab02e99d2a599a4ca13bdb55b4ef7fadafa4140f
...
# emacs23 obsoletes emacs21 and emacs22
...
[ -f doc/lispintro/configure ] || cp %SOURCE70 doc/lispintro/configure
[ -f doc/lispref/configure ] || cp %SOURCE71 doc/lispref/configure
...
# We build four binaries (with X and without X support) 
...
%if_enabled nox
pushd build-nox
%configure --sharedstatedir=/var --with-pop --with-x=no --without-sound --with-gpm --without-dbus --without-rsvg
popd
%endif
%if_enabled athena
pushd build-athena
%configure --sharedstatedir=/var --with-pop --with-x-toolkit=athena --with-png --with-jpeg --with-xpm --with-gif --with-tiff --enable-font-backend --with-freetype --with-xft --with-dbus --without-rsvg
popd
%endif
%if_enabled gtk
pushd build-gtk
%configure --sharedstatedir=/var --with-pop --with-x-toolkit=gtk --with-png --with-jpeg --with-xpm --with-gif --with-tiff --with-gpm --enable-font-backend --with-freetype --with-xft --with-dbus --with-rsvg
popd
%endif
%if_enabled motif
...
pushd build-motif
%configure --sharedstatedir=/var --with-pop --with-x-toolkit=motif --with-png --with-jpeg --with-xpm --with-gif --with-tiff --enable-font-backend --with-freetype --with-xft --with-dbus --without-rsvg
popd
%endif

# MH-E autoloads fixed
# mkdir -p build-nox/lisp/mh-e
# mkdir -p build-athena/lisp/mh-e
# mkdir -p build-motif/lisp/mh-e
# mkdir -p build-gtk/lisp/mh-e
...
# FIXME printable formats of elisp-manual
# pushd doc/lispintro
...
#####################
# INFO
%define common_infos ada-mode,auth,autotype,calc,ccmode,cl,dbus,dired-x,ebrowse,ediff,edt,efaq,%shortname,epa,eshell,eudc,flymake,forms,idlwave,mairix-el,mh-e,newsticker,org,pcl-cvs,pgg,rcirc,reftex,remember,sasl,sc,ses,smtpmail,url,vip,viper,widget,woman
...
#
%files elisp-manual
# %doc doc/lispintro/emacs-lisp-intro.dvi doc/lispref/elisp.dvi
%_infodir/eintr*
%_infodir/elisp*
