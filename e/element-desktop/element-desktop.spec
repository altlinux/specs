%define elm_desktop element.desktop
%define del_list delete_list.txt
...
# auto predownloaded node modules during update version with 
# rpmgs from etersoft-build-utils
# ask me about description using: lav@etersoft.ru
Source1: %name-development-%version.tar

Source3: %elm_desktop
Source4: hak.tar.gz
Source5: prod_node_modules.tar.gz
Source6: %del_list



AutoReq:yes,nonodejs,nonodejs_native,nomono,nopython
AutoReq:nomingw32,nomingw64,noshebang
#AutoProv: no
...
Requires: electron23
BuildRequires: electron23 node-asar

BuildRequires: rust rust-cargo libsqlcipher-devel python3

Provides: riot-desktop = %version-%release
Obsoletes: riot-desktop

BuildRequires: node-typescript node-gyp libsecret-devel

%description
Element Desktop is a Matrix client for desktop platforms with
Element Web at its core.

%define build_root %_builddir/%name-%version
%define arch_target %_arch-unknown-linux-gnu
%define cargo_root %build_root/.hak/matrix-seshat/%arch_target/build/native/.cargo
...
tar -C %build_root -zxf %SOURCE4
cp %SOURCE6 %build_root/
...
%build
npm config set python %__python3
export CARGO_HOME=%cargo_root
yarn run --offline hak build
yarn run --offline hak copyandlink
yarn run --offline build:ts
yarn run --offline build:res
#npm ERR! cb() never called!
#npm ERR! This is an error with npm itself. Please report this error at:
# Replace links by reall folders

rm -fr node_modules
tar -C %build_root -zxf %SOURCE5

#Resolve links
cp -fr .hak/hakModules/keytar node_modules/
cp -fr .hak/hakModules/matrix-seshat node_modules/

#Remove development dependancies
cat ./%del_list | xargs rm -vfr
ls -h
...
cat <<EOF >%name
#!/bin/sh
electron23 %_datadir/%name/resources/app.asar "\$@"
EOF
...
install -D -m644 %SOURCE3 %buildroot%_desktopdir/%elm_desktop

%files
%_bindir/riot-desktop
%_bindir/%name
%_datadir/%name/
%_desktopdir/%elm_desktop
