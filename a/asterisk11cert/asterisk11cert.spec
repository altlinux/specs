...
%add_verify_elf_skiplist %_libdir/libasteriskssl%version.so.1
%def_with debug
%def_enable debug
%def_without		hoard
%def_without 		addons
...
%define ast_doc_subdir  %version
%define ast_doc_dir     /usr/share/asterisk/documentation/%ast_doc_subdir
%define astmodule() \
...
%package codec_opus
Summary: OPUS codec support for Asterisk
Group: %group
Requires: %name = %version-%release

%description codec_opus
OPUS codec support for Asterisk PBX
...
Group: %group
BuildArch: noarch
Requires: aelparse11cert = %version-%release
...
Requires: %name-format_vp8   = %version-%release
Requires: %name-codec_opus   = %version-%release
Requires: conf2ael11cert
...
%package format_vp8
Summary: VP8 format for Asterisk
Group: System/Servers
PreReq: %name = %version-%release

%description format_vp8
VP8 format for Asterisk
...
%package -n conf2ael11cert
Summary: extensions.conf -> ael2 converter
Group: %group
Requires: asterisk11-common

%description -n conf2ael11cert
extensions.conf -> ael2 converter


%package -n libasteriskssl11cert
Summary: Asterisk SSL functions
Group: %group

%description -n libasteriskssl11cert
...
sed -i "s!documentation/!documentation/%ast_doc_subdir/!g" main/xmldoc.c
sed -i "s!documentation{!documentation/%ast_doc_subdir{!g" main/xmldoc.c
sed -i "s!ASTSSL_LDLIBS=-L. -lasteriskssl.*!ASTSSL_LDLIBS=-L. -lasteriskssl%version!" main/Makefile
sed -i "s!ASTSSL_LIB:=.*!ASTSSL_LIB:=libasteriskssl%version.so!" main/Makefile
...
mkdir -p %buildroot/%ast_doc_dir
mv %buildroot/var/lib/asterisk/documentation/*.xml %buildroot/%ast_doc_dir/
mv %buildroot/var/lib/asterisk/documentation/*.dtd %buildroot/%ast_doc_dir/
ln -sf libasteriskssl%version.so.1 %buildroot%_libdir/libasteriskssl%version.so
...
%dir %ast_doc_dir
%ast_doc_dir/appdocsxml.dtd
%ast_doc_dir/core-en_US.xml
...
%astmodule res_xmpp
%dir %_docdir/%name-%version/samples
%exclude %astsample jabber
%exclude %astsample jingle
%exclude %astsample gtalk
%astsample xmpp
%astsample motif
...
%files -n conf2ael11cert
%_sbindir/conf2ael-%version
%_altdir/conf2ael-%version

%files -n libasteriskssl11cert
