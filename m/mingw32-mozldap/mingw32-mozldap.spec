%define oname mozldap
%define major  6
%define minor  0
%define submin 7

Name:		mingw32-mozldap
Version:	6.0.7
Release:	alt2
...
Source0:	ftp://ftp.mozilla.org/pub/mozilla.org/directory/c-sdk/releases/v%version/src/mozldap.tar
...
Patch10:	mozldap-alt-mingw-fix.patch
...
BuildRequires:	mingw32-libnspr-devel
BuildRequires:	mingw32-libnss-devel
...
%__subst 's|NSINSTALL=nsinstall||g' c-sdk/configure*

#__subst 's|LIB_PREFIX=lib|LIB_PREFIX=|g' c-sdk/configure*

# incorrectly set when --with-system-nspr and --with_system_nss
%__subst 's|\$(DYNAMICNSS)||g' c-sdk/build.mk
%__subst 's|\$(DYNAMICNSPR)||g' c-sdk/build.mk
...
#	`%{_mingw32_target}-pkg-config --cflags-only-I nspr` \
#	`%{_mingw32_target}-pkg-config --libs-only-L nspr`"
...
mkdir -p %buildroot/%_mingw32_bindir \
    %buildroot/%_mingw32_includedir/%oname \
    %buildroot/%_mingw32_libdir/
#    %buildroot/%_datadir/%name/etc \
#    #

# Copy the binary libraries we want
find dist/bin/ -name '*.dll' -execdir %__install -m 755 -t %buildroot/%_mingw32_bindir \{\} \;

#for file in nsldif32v60 nsldapssl32v60 nsldappr32v60 nsldap32v60
#do
#  install -m644 dist/bin/$file.dll %buildroot%{_mingw32_bindir}/${file}.dll
#  install -m644 dist/lib/lib$file.a %buildroot%{_mingw32_libdir}/lib${file}.a
#  install -m644 dist/lib/lib$file.dll.a %buildroot%{_mingw32_libdir}/lib${file}.dll.a
#done

find dist/lib/ -name '*.a' -execdir %__install -m 755 -t %buildroot/%_mingw32_libdir \{\} \;

# Copy the binaries we want
find dist/bin/ -name 'ldap*' |
while read f; do
    install -m 755 -- "$f" "%buildroot/%_mingw32_bindir/moz${f##*/}"
done

# Copy the include files
find dist/public/ldap -name '*.h' -execdir %__install -m 644 -t %buildroot/%_mingw32_includedir/%oname \{\} \;
#%SOURCE2 %_includedir %buildroot/%_includedir/%oname
...
sed \
    ...
    %buildroot/%_mingw32_libdir/pkgconfig/%oname.pc
...
%_mingw32_includedir/%oname/
%_mingw32_libdir/*.a
#%_datadir/%name

%changelog
* Sat Mar 26 2011 Vitaly Lipatov <lav@altlinux.ru> 6.0.7-alt2
- build for mingw
