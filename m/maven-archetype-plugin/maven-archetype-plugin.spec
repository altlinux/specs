Name: maven-archetype-plugin
Version: 2.3
Summary: Maven Plugin for using archetypes
License: ASL 2.0
Url: https://maven.apache.org/archetype/
Epoch: 0
Packager: Igor Vlasenko <viy@altlinux.ru>
Provides: maven-archetype-plugin = 2.3-2.fc23
Provides: mvn(org.apache.maven.plugins:maven-archetype-plugin) = 2.3
Provides: mvn(org.apache.maven.plugins:maven-archetype-plugin:pom:) = 2.3
Requires: java-headless
Requires: jpackage-utils
Requires: mvn(commons-collections:commons-collections)
Requires: mvn(commons-io:commons-io)
Requires: mvn(org.apache.maven.archetype:archetype-catalog)
Requires: mvn(org.apache.maven.archetype:archetype-common)
Requires: mvn(org.apache.maven.archetype:archetype-descriptor)
Requires: mvn(org.apache.maven.archetype:archetype-registry)
Requires: mvn(org.apache.maven.shared:maven-invoker)
Requires: mvn(org.apache.maven.shared:maven-script-interpreter)
Requires: mvn(org.apache.maven:maven-core)
Requires: mvn(org.apache.maven:maven-plugin-api)
Requires: mvn(org.beanshell:bsh)
Requires: mvn(org.codehaus.plexus:plexus-interactivity-api)
Requires: mvn(org.codehaus.plexus:plexus-utils)

BuildArch: noarch
Group: Development/Java
Release: alt0.1jpp
Source: maven-archetype-plugin-2.3-2.fc23.cpio

%description
Maven Plugin for using archetypes.

# sometimes commpress gets crazy (see maven-scm-javadoc for details)
%set_compress_method none
%prep
cpio -idmu --quiet --no-absolute-filenames < %{SOURCE0}

%build
cpio --list < %{SOURCE0} | sed -e 's,^\.,,' > %name-list

%install
mkdir -p $RPM_BUILD_ROOT
for i in usr var etc; do
[ -d $i ] && mv $i $RPM_BUILD_ROOT/
done


%files -f %name-list

%changelog
* Wed Jan 20 2016 Igor Vlasenko <viy@altlinux.ru> 0:2.3-alt0.1jpp
- bootstrap pack of jars created with jppbootstrap script
- temporary package to satisfy circular dependencies

