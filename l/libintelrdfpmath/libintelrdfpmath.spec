...
Patch0008: fexcept_t-conflict.patch
...
export CPPFLAGS="-fPIC -O1"

export CC=gcc
for CALL_BY_REF in 0 1; do
    for GLOBAL_RND in 0 1; do
	for GLOBAL_FLAGS in 0 1; do
	    %make_build -C LIBRARY CC=${CC} CC_NAME_INDEX=4 CC_INDEX=4 _HOST_ARCH=${_HOST_ARCH} CALL_BY_REF=${CALL_BY_REF} GLOBAL_RND=${GLOBAL_RND} GLOBAL_FLAGS=${GLOBAL_FLAGS} UNCHANGED_BINARY_FLAGS=0 CFLAGS="${CFLAGS}" CPPFLAGS="${CPPFLAGS}"
	    ...
	    %make_build -C LIBRARY CC=${CC} CC_NAME_INDEX=4 CC_INDEX=4 _HOST_ARCH=${_HOST_ARCH} CALL_BY_REF=${CALL_BY_REF} GLOBAL_RND=${GLOBAL_RND} GLOBAL_FLAGS=${GLOBAL_FLAGS} UNCHANGED_BINARY_FLAGS=1 CFLAGS="${CFLAGS}" CPPFLAGS="${CPPFLAGS}"
