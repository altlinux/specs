...
%global omptarget_name libomptarget%v_majmin
...
%global libomp_default_name libomp%_llvm_version
...
%if_without libomp
%add_findreq_skiplist %llvm_libdir/libomp.so
%endif
%add_findreq_skiplist %llvm_libdir/libompd.so
%add_findreq_skiplist %_bindir/llvm-offload-device-info-%v_major
...
%endif
%if_without libomp
BuildRequires: %libomp_default_name-devel
%endif
...
%package -n %omptarget_name
Summary: Clang runtime for OpenMP target offloading
Group: System/Libraries
Requires: %clang_name-support = %EVR
%requires_filesystem

%description -n %omptarget_name
...
%package -n libomp%omp_vmajor
Summary: LLVM/clang OpenMP library
Group: System/Libraries
%requires_filesystem

%description -n libomp%omp_vmajor
LLVM/clang OpenMP library.

%package -n lib%omp_name-devel
Summary: OpenMP header files
Group: Development/C
Requires: %clang_name = %EVR %{llvm_name}-devel = %EVR %{clang_name}-tools = %EVR %lld_name = %EVR
%requires_filesystem
...
%endif
	...
	\
	%ifarch %libomptarget_arches
	-DRUNTIMES_nvptx64-nvidia-cuda_LLVM_ENABLE_RUNTIMES="$target_runtimes" \
	-DRUNTIMES_amdgcn-amd-amdhsa_LLVM_ENABLE_RUNTIMES="$target_runtimes" \
...
%if_with lld
cp -a lld/include/mach-o %buildroot%llvm_includedir/
%endif
...
# tests are not enabled so removing
rm -f %buildroot%llvm_man1dir/llvm-test-mustache-spec*
...
%if_with libomp
mv %buildroot%llvm_libdir/libomp.so.%omp_vmajor %buildroot%_libdir/
%else
rm -f %buildroot%llvm_libdir/libomp.so*
ln -srv %buildroot%_libdir/libomp.so.%omp_vmajor %buildroot%llvm_libdir/libomp.so
%endif
ln -srv %buildroot%_libdir/libomp.so.%omp_vmajor %buildroot%llvm_libdir/libomp.so.%omp_vmajor
%ifarch %libomptarget_arches
...
# libompdevice lib is expected to be in llvm_libdir/clang/%%v_major/lib/
mv %buildroot%llvm_libdir/{amdgcn-amd-amdhsa,nvptx64-nvidia-cuda} %buildroot%llvm_libdir/clang/%v_major/lib/
%endif
%ifarch %libc_arches
cp -a %buildroot%llvm_prefix/lib/nvptx64-nvidia-cuda/* %buildroot%llvm_libdir/clang/%v_major/lib/nvptx64-nvidia-cuda/
rm -rf %buildroot%llvm_prefix/lib/{clang,nvptx64-nvidia-cuda}
...
/APPEND _cmake_import_check_targets \(llvm-offload-device-info\|llvm-omp-kernel-replay\|llvm-gpu-loader\|omptarget\)/ {s|^|#|}
/APPEND _cmake_import_check_targets \(omp\)/ {s|^|#|}
/APPEND _cmake_import_check_targets \(LLVMOffload\)/ {s|^|#|}
...
%exclude %llvm_libdir/clang/%v_major/lib/%libomp_arch-unknown-linux-gnu/libLLVMOffload.so*
...
%ifarch %libomptarget_arches
%files -n %omptarget_name
%llvm_libdir/clang/%v_major/lib/%libomp_arch-unknown-linux-gnu/libomptarget.so.%v_majmin
%_libdir/libomptarget.so.%v_majmin
%llvm_libdir/libLLVMOffload.so.%v_majmin
%_libdir/libLLVMOffload.so.%v_majmin
%endif

%if_with libomp
%files -n libomp%omp_vmajor
...
%ifarch %libomptarget_arches
%llvm_includedir/offload
%llvm_libdir/libLLVMOffload.so
...
%changelog
* Wed Apr 29 2026 L.A. Kostis <lakostis@altlinux.ru> 21.1.8-alt0.4
- don't package libomp (use the system one).
- remove libarcher.so from system paths.

* Wed Apr 29 2026 L.A. Kostis <lakostis@altlinux.ru> 21.1.8-alt0.3
- OpenMP support overhaul:
  - split out omptarget library into separate package
  - provide libomp library according shared libs policy.
...
* Tue Oct 14 2025 L.A. Kostis <lakostis@altlinux.ru> 21.1.3-alt0.1
- Update to 21.1.3.
...
* Mon Jun 30 2025 L.A. Kostis <lakostis@altlinux.ru> 20.1.7-alt0.1
- Update to 20.1.7.
