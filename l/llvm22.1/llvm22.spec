...
# https://github.com/llvm/llvm-project/issues/181598
# change below should be reverted
Patch109: llvm-add-lib-suffix-PR159758.patch
...
BuildRequires: zip zlib-devel binutils-devel libzstd-devel
...
%patch108 -p2
#%%patch109 -p1 -R -b .llvm-add-lib-suffix
...
%build
PROJECTS="clang;clang-tools-extra;mlir;polly"
RUNTIMES="compiler-rt"
# for openmp offload
# https://github.com/llvm/llvm-project/issues/106399
%ifarch %libomptarget_arches
RUNTIMES="$RUNTIMES;openmp;offload"
target_runtimes=openmp
%else
RUNTIMES="$RUNTIMES;openmp"
%endif
%ifarch %libc_arches
target_runtimes="$target_runtimes;libc"
%endif
%if_with lld
...
# compiler-rt clang builtins
%ifnarch %ix86
mv %buildroot%llvm_prefix/lib/clang/%v_major/lib/%_arch-unknown-linux-gnu/* %buildroot%llvm_libdir/clang/%v_major/lib/%_arch-unknown-linux-gnu/
rm -rf %buildroot%llvm_prefix/lib/clang
%endif
...
mv %buildroot%llvm_prefix/lib/nvptx64-nvidia-cuda %buildroot%llvm_libdir/clang/%v_major/lib/
mv %buildroot%llvm_libdir/amdgcn-amd-amdhsa %buildroot%llvm_libdir/clang/%v_major/lib/
%endif
%ifarch %libc_arches
...
%if_with lldb
emit_filelist >%_tmppath/dyn-files-%lldb_name <<EOExecutableList
bin,man	lldb
bin	lldb-argdumper
bin	lldb-instr
bin,man	lldb-server
...
/APPEND _cmake_import_check_targets \(llvm-offload-device-info\|llvm-offload-binary\|llvm-offload-wrapper\|llvm-omp-kernel-replay\|llvm-gpu-loader\|omptarget\)/ {s|^|#|}
...
%ifarch %libc_arches
%exclude %llvm_libdir/clang/%v_major/include/llvm_libc_wrappers
%endif
%ifarch %libomptarget_arches
%exclude %llvm_libdir/clang/%v_major/lib/amdgcn-amd-amdhsa
%exclude %llvm_libdir/clang/%v_major/lib/nvptx64-nvidia-cuda
%exclude %llvm_libdir/clang/%v_major/include/llvm_offload_wrappers
...
%if_with lldb
%files -n %lldb_name -f %_tmppath/dyn-files-%lldb_name
...
%_libdir/libMLIR.so.*
%llvm_libdir/libmlir_apfloat_wrappers.so.*
...
%llvm_libdir/clang/%v_major/include/llvm_offload_wrappers
...
%llvm_libdir/clang/%v_major/include/llvm_libc_wrappers
%endif
...
* Wed Apr 29 2026 L.A. Kostis <lakostis@altlinux.ru> 22.1.3-alt0.3
- Provide libomp.so.5 as system library via separate package.

* Wed Apr 29 2026 L.A. Kostis <lakostis@altlinux.ru> 22.1.3-alt0.2
- OpenMP/offload changes:
  + do not build libomp.so, use default one instead (should fix soversion
    clashes between different llvm versions).
  + move omptarget/offload libs into separate package.

* Sat Apr 18 2026 L.A. Kostis <lakostis@altlinux.ru> 22.1.3-alt0.1
- 22.1.3.
