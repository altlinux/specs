...
select distinct a.pkgid from rpm_files as a where (a.filename glob '/lib/systemd/*' or a.filename glob '/usr/lib/systemd/*') and a.pkgid not in (select c.pkgid from rpm_files as c where c.filename glob '/etc/rc.d/init.d/*');
EOSQL
