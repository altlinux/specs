...
for sub in bundleutils finder reflect naming classpath spring; do
    # install jar
    install -Dpm 644 %{oldname}-${sub}/target/%{oldname}-${sub}-%{version}.jar \
            $RPM_BUILD_ROOT/%{_javadir}/%{name}/%{name}-${sub}.jar;

    # intall pom
    install -Dpm 644 %{oldname}-${sub}/pom.xml $RPM_BUILD_ROOT/%{_mavenpomdir}/JPP.%{name}-%{name}-${sub}.pom

    # maven depmap
    %add_to_maven_depmap org.apache.xbean %{oldname}-${sub} %{version} JPP/%{name} %{name}-${sub}
done


    # install jar
    install -Dpm 644 maven-xbean-plugin/target/maven-xbean-plugin-%{version}.jar \
            $RPM_BUILD_ROOT/%{_javadir}/%{name}/maven-xbean-plugin.jar;
...
%add_to_maven_depmap org.apache.xbean %{oldname} %{version} JPP/%{name} main
...
%changelog
* Tue Sep 04 2012 Igor Vlasenko <viy@altlinux.ru> 0:3.7-alt4_7jpp7
- compat build
