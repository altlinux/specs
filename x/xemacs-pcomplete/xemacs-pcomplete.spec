Name: xemacs-pcomplete
Version: 1.06
Release: alt1

%define_xemacs_package pcomplete

Summary: Programmatic completions for XEmacs
License: GPL
Group: Editors

%description
Programmatic completions for XEmacs

%prep
%setup -qc

%install
%xemacs_package_install
%xemacs_package_find_files

%files -f %name-files

%changelog
* Tue May 03 2011 Sergey Bolshakov <sbolshakov@altlinux.ru> 1.06-alt1
- 1.06

* Thu May 21 2009 Sergey Bolshakov <sbolshakov@altlinux.ru> 1.05-alt1
- 1.05

* Sat Mar 12 2005 Sergey Bolshakov <sbolshakov@altlinux.ru> 1.04-alt1
- 1.04

* Sat Nov 15 2003 Sergey Bolshakov <sbolshakov@altlinux.ru> 1.03-alt1
- 1.03

* Sat Dec 21 2002 Sergey Bolshakov <s.bolshakov@belcaf.com> 1.02-alt2
- #1531 fixed

* Mon Oct 28 2002 Sergey Bolshakov <s.bolshakov@belcaf.com> 1.02-alt1
- 1.02

* Mon Aug  5 2002 Sergey Bolshakov <s.bolshakov@belcaf.com> 1.01-alt1
- first build for %distribution distribution

