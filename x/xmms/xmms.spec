...
X MultiMedia System - колись популярний медіа-програвач для UNIX-систем.

Підтримує величезну кількість форматів та ефектів завдяки великій кількості
модулів розширення, що доступні в окремих пакетах.

Цей пакунок зібраний з патчами rusxmms.sf.net, див. також
http://dside.dyndns.org/rusxmms/wiki/FAQ

%package -n libxmms
Summary: Library needed for XMMS and its plugins
Summary(ru_RU.UTF-8): Библиотека для XMMS и модулей к нему
Summary(uk_UA.UTF-8): Бібліотека для XMMS та модулів до нього
...
Ця бібліотека є необхідною для xmms та усіх його модулей.
...
Summary(ru_RU.UTF-8): Модуль поддержки воспроизведения MOD-файлов
Summary(uk_UA.UTF-8): Модуль підтримки відтворення MOD-файлів
Group: Sound
...
Модуль вводу для програвання MOD-файлів (.mod, .xm, .s3m, ...)

%package in-vorbis
Summary: Input plugin that uses the Vorbis library
Summary(ru_RU.UTF-8): Модуль поддержки воспроизведения Ogg Vorbis
Summary(uk_UA.UTF-8): Модуль підтримки відтворення Ogg Vorbis
Group: Sound
...
Модуль вводу, що використовує бібліотеку Ogg Vorbis

%package out-alsa
Summary: ALSA output plugin
Summary(ru_RU.UTF-8): Модуль вывода для ALSA
Summary(uk_UA.UTF-8): Модуль виводу для ALSA
Group: Sound
...
Модуль виводу для використання із ALSA

%package out-diskwriter
Summary: DiskWriter output plugin
Summary(ru_RU.UTF-8): Модуль вывода в файл
Summary(uk_UA.UTF-8): Модуль виводу в файл
Group: Sound
Requires: %name = %epoch:%version-%release
...
Модуль виводу для запису wav-файлів замість відтворення

%package out-esd
Summary: ESound output plugin
Summary(ru_RU.UTF-8): Модуль вывода через ESound
Summary(uk_UA.UTF-8): Модуль виводу через ESound
Group: Sound
...
Модуль виводу, що застосовується при необхідності використання ESound

%package vis-mesa
Summary: Visualization plugins that use the Mesa3D library
Summary(ru_RU.UTF-8): Модули визуализации с использованием Mesa3D
Summary(uk_UA.UTF-8): Модулі візуалізації з використанням Mesa3D
Group: Sound
...
Модуль візуалізації з використанням 3D-ефектів
...
На відміну від wmusic, не підтримує кирилицю.

%package cyr-setup
Summary: Script to auto-tune XMMS for cyrillic tags
Summary(ru_RU.UTF-8): Скрипт для автонастройки XMMS под кириллицу
Summary(uk_UA.UTF-8): Скрипт для автоналаштування XMMS під кирилицю
