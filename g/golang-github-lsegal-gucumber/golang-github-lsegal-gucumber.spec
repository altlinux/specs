...
%global project         gucumber
%global repo            gucumber
# https://github.com/gucumber/gucumber
%global provider_prefix %{provider}.%{provider_tld}/%{project}/%{repo}
%global import_path     %{provider_prefix}
%global sec_import_path github.com/lsegal/gucumber
...
chmod -x LICENSE.txt README.md runner.go

%build

%install
find . -iname "*.go" -exec chmod -x {} \;
chmod -x README.md LICENSE.txt
# source codes for building projects
%if 0%{?with_devel}
install -d -p %{buildroot}/%{go_path}/src/%{import_path}/
install -d -p %{buildroot}/%{go_path}/src/%{sec_import_path}/
echo "%%dir %%{go_path}/src/%%{import_path}/." >> devel.file-list
echo "%%dir %%{go_path}/src/%%{sec_import_path}/." >> devel.file-list
...
for file in $(find . -iname "*.go" \! -iname "*_test.go") ; do
    ...
    while [ ${filedir%%/*} != "$filedir" ]; do
        filedir=${filedir%%/*}
	echo "%%dir %%{go_path}/src/%%{sec_import_path}/$filedir" >> devel.file-list.dir
    done
done
