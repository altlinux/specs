...
install -d -p %{buildroot}/%{go_path}/src/%{gopkg_import_path}/
echo "%%dir %%{go_path}/src/%%{gopkg_import_path}/." >> devel.file-list
install -d -p %{buildroot}/%{go_path}/src/%{sec_gopkg_import_path}/
echo "%%dir %%{go_path}/src/%%{sec_gopkg_import_path}/." >> devel.file-list
...
for file in $(find . -iname "*.go" \! -iname "*_test.go") ; do
    ...
    echo "%%dir %%{go_path}/src/%%{sec_import_path}/$(dirname $file)" >> devel.file-list
    ...
    echo "%%dir %%{go_path}/src/%%{gopkg_import_path}/$(dirname $file)" >> devel.file-list
    install -d -p %{buildroot}/%{go_path}/src/%{gopkg_import_path}/$(dirname $file)
    cp -pav $file %{buildroot}/%{go_path}/src/%{gopkg_import_path}/$file
    echo "%%{go_path}/src/%%{gopkg_import_path}/$file" >> devel.file-list

    echo "%%dir %%{go_path}/src/%%{sec_gopkg_import_path}/$(dirname $file)" >> devel.file-list
    install -d -p %{buildroot}/%{go_path}/src/%{sec_gopkg_import_path}/$(dirname $file)
    cp -pav $file %{buildroot}/%{go_path}/src/%{sec_gopkg_import_path}/$file
    echo "%%{go_path}/src/%%{sec_gopkg_import_path}/$file" >> devel.file-list
done
%endif
...
%changelog
* Sat Dec 09 2017 Igor Vlasenko <viy@altlinux.ru> 1.3.1-alt1_0.4.gitf12c623
- new version
