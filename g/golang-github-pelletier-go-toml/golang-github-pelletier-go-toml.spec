...
%global commit          5ccdfb18c776b740aecaf085c4d9a2779199c279
...
Requires:      golang(github.com/pelletier/go-buffruneio)
...
for file in $(find . -iname "*_test.go" -or -iname "*.toml") ; do
    dirprefix=$(dirname $file)
    install -d -p %{buildroot}/%{go_path}/src/%{import_path}/$dirprefix
    cp -pav $file %{buildroot}/%{go_path}/src/%{import_path}/$file
...
%gotest %{import_path}
%gotest %{import_path}/cmd/tomljson
%endif
...
%changelog
* Sat Dec 09 2017 Igor Vlasenko <viy@altlinux.ru> 1.0.0-alt1_0.3.git5ccdfb1
- new version
