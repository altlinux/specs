...
%if 0%{?with_unit_test} && 0%{?with_devel}
install -d -p %{buildroot}/%{go_path}/src/%{import_path}/
install -d -p %{buildroot}/%{go_path}/src/%{import_path}/quantile
echo "%%dir %%{go_path}/src/%%{import_path}/./quantile" >> devel.file-list

cp -pav quantile/exampledata.txt %{buildroot}/%{go_path}/src/%{import_path}/quantile/.
echo "%%{go_path}/src/%%{import_path}/quantile/exampledata.txt" >> unit-test.file-list
...
%changelog
* Sat Dec 09 2017 Igor Vlasenko <viy@altlinux.ru> 0-alt1_0.10.gitd9a9656
- new version
