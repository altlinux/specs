# BEGIN SourceDeps(oneline):
BuildRequires(pre): rpm-build-perl
BuildRequires: perl(AnyEvent.pm) perl(AnyEvent/Handle.pm) perl(AnyEvent/Socket.pm) perl(App/Daemon.pm) perl(Archive/Tar.pm) perl(CHI.pm) perl(Catalyst/Runtime.pm) perl(Class/C3.pm) perl(Compress/Bzip2.pm) perl(Config.pm) perl(Data/Structure/Util.pm) perl(DateTime.pm) perl(DateTime/Format/DateParse.pm) perl(DateTime/Format/Mail.pm) perl(DateTime/Format/Natural.pm) perl(DateTime/Format/Strptime.pm) perl(Directory/Scratch.pm) perl(Email/Sender/Simple.pm) perl(Email/Sender/Transport/SMTP.pm) perl(Email/Simple.pm) perl(English.pm) perl(File/Find/Rule.pm) perl(File/MimeInfo/Magic.pm) perl(File/ShareDir.pm) perl(File/Slurp.pm) perl(File/Type.pm) perl(File/stat.pm) perl(FindBin.pm) perl(HTML/Mason.pm) perl(HTTP/Daemon.pm) perl(HTTP/Status.pm) perl(Hash/Merge.pm) perl(Hash/Merge/Simple.pm) perl(IO/Scalar.pm) perl(IO/Select.pm) perl(IO/Socket.pm) perl(IO/String.pm) perl(IPC/Open3.pm) perl(JSON.pm) perl(LWP/UserAgent.pm) perl(MRO/Compat.pm) perl(Memoize.pm) perl(Moose/Role.pm) perl(Net/OpenSSH.pm) perl(Perl6/Junction.pm) perl(Pod/Usage.pm) perl(Scalar/Util.pm) perl(Socket.pm) perl(Sub/Exporter.pm) perl(Sys/Hostname.pm) perl(Sys/Syslog.pm) perl(TAP/Parser.pm) perl(TAP/Parser/Aggregator.pm) perl(Template.pm) perl(Template/Stash.pm) perl(Test/Deep.pm) perl(Test/Differences.pm) perl(Test/Exception.pm) perl(Test/MockModule.pm) perl(Test/WWW/Mechanize/Catalyst.pm) perl(Text/Balanced.pm) perl(Try/Tiny.pm) perl(UNIVERSAL.pm) perl(URI/Escape.pm) perl(XML/Feed.pm) perl(XML/Simple.pm) perl(YAML/Syck.pm) perl(YAML/Tiny.pm) perl(YAML/XS.pm) perl(aliased.pm) perl(common/sense.pm) perl(namespace/autoclean.pm) perl(parent.pm) perl(subs.pm) perl-devel perl-podlators
# END SourceDeps(oneline)
%define upstream_name    Tapper-TestSuite-Netperf
%define upstream_version 4.1.1

Name:       perl-%{upstream_name}
Version:    4.1.1
Release:    alt1

Summary:    Tapper - Network performance measurements - Client
License:    GPL+ or Artistic
Group:      Development/Perl
Url:        http://search.cpan.org/dist/%{upstream_name}
Source:    http://www.cpan.org/authors/id/T/TA/TAPPER/Tapper-TestSuite-Netperf-%{version}.tar.gz

BuildRequires: perl(File/Temp.pm)
BuildRequires: perl(IO/Handle.pm)
BuildRequires: perl(IO/Socket/INET.pm)
BuildRequires: perl(Log/Log4perl.pm)
BuildRequires: perl(Moose.pm)
BuildRequires: perl(MooseX/Log/Log4perl.pm)
BuildRequires: perl(Test/More.pm)
BuildRequires: perl(YAML.pm)
BuildRequires: perl(strict.pm)
BuildRequires: perl(warnings.pm)
BuildArch: noarch
Source44: import.info

%description
Network performance measurements client for Tapper.

%prep
%setup -q -n %{upstream_name}-%{upstream_version}

%build
%{__perl} Makefile.PL INSTALLMAN1DIR=%_man1dir INSTALLMAN1DIR=%_man1dir INSTALLDIRS=vendor

%make

%check
%make test

%install
%makeinstall_std

%files
%doc META.json META.yml Changes LICENSE README
%perl_vendor_privlib/*
/usr/bin/tapper-testsuite-netperf-client
/usr/bin/tapper-testsuite-netperf-server
/usr/share/man/man1/tapper-testsuite-netperf-client.1.*
/usr/share/man/man1/tapper-testsuite-netperf-server.1.*



%changelog
* Sat Jul 27 2013 Igor Vlasenko <viy@altlinux.ru> 4.1.1-alt1
- automated CPAN update

* Mon Dec 10 2012 Igor Vlasenko <viy@altlinux.ru>  4.1.0-alt1_2
- mageia import by cas@ requiest

