%define oname Acquisition
Name: python-module-%oname
Version: 4.1
Release: alt1.git20141218
Summary: Zope implements acquisition with "Extension Class" mix-in classes
License: ZPLv2.1
Group: Development/Python
Url: http://pypi.python.org/pypi/Acquisition/
Packager: Eugeny A. Rostovtsev (REAL) <real at altlinux.org>

# https://github.com/zopefoundation/Acquisition.git
Source: %name-%version.tar

BuildPreReq: python-devel python-module-setuptools-tests
BuildPreReq: python-module-Zope2-tests

%py_requires ExtensionClass zope.interface

%description
Acquisition is a mechanism that allows objects to obtain attributes from
the containment hierarchy they're in.

%package tests
Summary: Tests for Acquisition
Group: Development/Python
Requires: %name = %version-%release

%description tests
Acquisition is a mechanism that allows objects to obtain attributes from
the containment hierarchy they're in.

This package contains tests for Acquisition.

%prep
%setup

%build
%python_build

%install
%python_install

%check
python setup.py test

%files
%doc *.txt *.rst
%python_sitelibdir/*
%exclude %python_sitelibdir/*/tests.*

%files tests
%python_sitelibdir/*/tests.*

%changelog
* Fri Dec 19 2014 Eugeny A. Rostovtsev (REAL) <real at altlinux.org> 4.1-alt1.git20141218
- Version 4.1

* Tue Nov 04 2014 Eugeny A. Rostovtsev (REAL) <real at altlinux.org> 4.0.4-alt1.git20141102
- Version 4.0.4

* Wed Oct 15 2014 Eugeny A. Rostovtsev (REAL) <real at altlinux.org> 4.0.1-alt1.dev.git20130321
- Version 4.0.1dev
- Enabled testing

* Mon Sep 16 2013 Eugeny A. Rostovtsev (REAL) <real at altlinux.org> 4.0-alt2
- Version 4.0

* Fri Sep 21 2012 Eugeny A. Rostovtsev (REAL) <real at altlinux.org> 4.0-alt1.a1
- Version 4.0a1

* Thu Apr 12 2012 Vitaly Kuznetsov <vitty@altlinux.ru> 2.13.7-alt2.1.1
- Rebuild to remove redundant libpython2.7 dependency

* Thu Oct 20 2011 Vitaly Kuznetsov <vitty@altlinux.ru> 2.13.7-alt2.1
- Rebuild with Python-2.7

* Tue Jun 21 2011 Eugeny A. Rostovtsev (REAL) <real at altlinux.org> 2.13.7-alt2
- Added necessary requirements

* Mon May 30 2011 Eugeny A. Rostovtsev (REAL) <real at altlinux.org> 2.13.7-alt1
- Initial build for Sisyphus

